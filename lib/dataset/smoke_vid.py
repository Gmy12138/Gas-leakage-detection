...
class SmokeVID(IMDB):
    ...
    # def __init__(self, image_set, root_path, dataset_path, result_path=None):
        ...
        super(SmokeVID, self).__init__('SmokeVID', image_set, root_path, dataset_path,
    ...
    def image_path_from_index(self, index):
        ...
        """
        if self.det_vid == 'DET':
            image_file = os.path.join(self.data_path, 'Data', 'DET', index + '.png')
        else:
            image_file = os.path.join(self.data_path, 'Data', 'VID', index + '.png')
    ...
    def evaluate_detections_multiprocess_seqnms(self, detections, gpu_id):
        ...
        # print 'ggg'
        # print detections
        self.write_vid_results_mult(detections, gpu_id)
        # print 'mmm'
        return 1
    ...
    def get_result_file_temp(self, gpu_id):
        """
    ...
    def write_vid_results_mult(self, detection, gpu_id):
        """
        ...
        filename = self.get_result_file_temp(gpu_id).format('all')
    ...
    def do_python_eval_gen(self, gpu_number=None):
        ...
        if gpu_number != None:
            ...
            for i in range(gpu_number):
                filename = self.get_result_file_temp(i).format('all')
        ...
        if self.enable_detailed_eval:
            ...
            motion_ranges = [[0.0, 1.0], [0.0, 0.8], [0.8, 0.9], [0.9, 1.0]]

            # motion_ranges = [[0.0, 1.0]]
            # area_ranges = [[0, 1e5 * 1e5], [0, 80 * 80], [80 * 80, 150 * 150], [150 * 150, 1e5 * 1e5]]
...
if __name__ == '__main__':
    ...
    # print(curr_path)
    dataset = 'SmokeVID'
    dataset_path = "/home/ecust/gmy/mxnet_code/Deep-Feature-Flow/data/smoke"
    image_set = 'VID_train_smoke'
    root_path = "/home/ecust/gmy/mxnet_code/Deep-Feature-Flow/data"
    test_image_set = 'VID_val_smoke'
    imdb = SmokeVID(image_set, root_path, dataset_path)

    # imdb.load_image_set_index()
    print imdb.image_set_index[0]
    print imdb.pattern[0]
    print imdb.frame_id[0]
    print imdb.frame_seg_id[0]
    print imdb.frame_seg_len[0]
    class_to_index = dict(zip(imdb.classes_map, range(imdb.num_classes)))
